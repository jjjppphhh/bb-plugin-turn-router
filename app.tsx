...
function RouterSettings(){
  ...
  return <form className="turn-router-settings" onSubmit={async e=>{e.preventDefault();try{setValue(await rpc.call('updateSettings',value));setStatus('Saved');}catch{setStatus('Could not save settings.');}}}>
    ...
    <label>Classifier<select value={value.classifier} onChange={e=>setValue({...value,classifier:e.target.value as Settings['classifier']})}><option value="luna">Luna · Existing Codex Sign-In</option><option value="jev">Jev · Fast Typed Decisions</option><option value="compatible-api">Compatible API · DeepSeek, Kimi, or Another Provider</option></select></label>
    {value.classifier==='luna'?<label>Codex Executable<input value={value.codexBinary} onChange={e=>setValue({...value,codexBinary:e.target.value})}/></label>:value.classifier==='jev'?<><p>TypeSafe receives the draft and a filtered recent conversation excerpt. Standard requests are processed in the United States and are not covered by zero-data-retention unless your TypeSafe plan provides it. Add the TypeSafe key in the secure plugin field.</p><label>Pinned Jev Model<input value={value.jevModel} pattern="jev-\d+\.\d+\.\d+" onChange={e=>setValue({...value,jevModel:e.target.value})}/></label></>:<><p>Your selected provider receives the draft and a short recent conversation excerpt. Add its key in the secure plugin field.</p><label>HTTPS API Base URL<input value={value.apiBaseUrl} placeholder="https://provider.example/v1" onChange={e=>setValue({...value,apiBaseUrl:e.target.value})}/></label><label>Classifier Model ID<input value={value.classifierModel} onChange={e=>setValue({...value,classifierModel:e.target.value})}/></label></>}
