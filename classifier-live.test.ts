...
it.skipIf(process.env.TURN_ROUTER_LIVE!=='1')('rates a synthetic task using the existing Luna sign-in',async()=>{
  ...
  const result=await classify({text:'Change the Save button label to Save changes.',context:'The settings screen is implemented and its tests pass.',attachments:0},{...defaults,timeoutMs:30000});
